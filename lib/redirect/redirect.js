...
const fallbackRedirects = [internetExplorerRedirect, ...staticRedirects];
...
module.exports = async () => {
    ...
    try {
        ...
        if (!redirectsRes.ok) {
            console.error('Redirects - CMS responded with', redirectsRes.status, '- using static redirects only.');
            return fallbackRedirects;
        }

        const redirectsData = await redirectsRes.json();
        docs = redirectsData.docs ?? [];
    } catch (error) {
        console.error('Redirects - Failed to load CMS redirects, using static redirects only.', error);
        return fallbackRedirects;
    }
